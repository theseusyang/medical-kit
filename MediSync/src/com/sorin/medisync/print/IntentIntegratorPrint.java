...
import com.sorin.medisync.qr.IntentResult;
 ...
 * @author gcstang
 */
public class IntentIntegratorPrint {

	public static final int REQUEST_CODE = 0x0000c0de; // Only use bottom 16
														// bits
	private static final String TAG = IntentIntegratorPrint.class
			.getSimpleName();
	...
	public static final List<String> TARGET_GOOGLE_CLOUD_PRINT = Collections
			.singletonList(GOOGLE_CLOUD_PRINT_PACKAGE);
	...
	public IntentIntegratorPrint(Activity activity) {
		this.activity = activity;
		title = DEFAULT_TITLE;
		message = DEFAULT_MESSAGE;
		buttonYes = DEFAULT_YES;
		buttonNo = DEFAULT_NO;
		targetApplications = TARGET_GOOGLE_CLOUD_PRINT;
	...
	public AlertDialog initiateScan(Collection<String> desiredBarcodeFormats) {
		Intent intentScan = new Intent(GOOGLE_CLOUD_PRINT_PACKAGE);
	...
	public AlertDialog shareText(CharSequence text, CharSequence type) {
		...
		intent.setAction(GOOGLE_CLOUD_PRINT_PACKAGE);

		String targetAppPackage = findTargetAppPackage(intent);
		if (targetAppPackage == null) {
			return showDownloadDialog();
		} else {
			Intent intentStart = new Intent(Intent.ACTION_SEARCH);
			intentStart.setPackage(targetAppPackage);
			intentStart.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
			intentStart.addFlags(Intent.FLAG_ACTIVITY_CLEAR_WHEN_TASK_RESET);
			attachMoreExtras(intentStart);
