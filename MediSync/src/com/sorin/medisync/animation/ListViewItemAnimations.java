...
package com.sorin.medisync.animation;
 ...
 * and how to use animations to complete the swipe as well as to animate the
 * other items in the list into their final places. This code works on runtimes
 * back to Gingerbread (Android 2.3), by using the android.view.animation
 * classes on earlier releases.
 * 
 * Watch the associated video for this demo on the DevBytes channel of
 * developer.android.com or on the DevBytes playlist in the androiddevelopers
...
public class ListViewItemAnimations extends Activity {
	...
	/**
	 * Animates a swipe of the item either back into place or out of the
	 * listview container. NOTE: This is a simplified version of swipe behavior,
	 * for the purposes of this demo about animation. A real version should use
	 * velocity (via the VelocityTracker class) to send the item off or back at
	 * an appropriate speed.
	 */
	@SuppressLint("NewApi")
	private void animateSwipe(final View view, float endX, long duration,
			final boolean remove) {
		mAnimating = true;
		mListView.setEnabled(false);
		if (isRuntimePostGingerbread()) {
			view.animate().setDuration(duration).alpha(remove ? 0 : 1)
					.translationX(endX)
			...
			TranslateAnimation swipeAnim = new TranslateAnimation(mCurrentX,
					endX, 0, 0);
			AlphaAnimation alphaAnim = new AlphaAnimation(mCurrentAlpha,
					remove ? 0 : 1);
	...
	private void setSwipePosition(View view, float deltaX) {
		...
		if (isRuntimePostGingerbread()) {
			...
			// Hello, Gingerbread!
			TranslateAnimation swipeAnim = new TranslateAnimation(deltaX,
					deltaX, 0, 0);
			mCurrentX = deltaX;
	...
	private void animateOtherViews(final ListView listview, View viewToRemove) {
		...
		// After layout runs, capture position of all itemIDs, compare to
		// pre-layout
			...
			public boolean onPreDraw() {
				...
				for (int i = 0; i < listview.getChildCount(); ++i) {
					...
					if (startTop == null) {
						...
						// starting position
						// based on whether they're coming in from the bottom (i
						// > 0) or top.
					...
					if (delta != 0) {
						Runnable endAction = firstAnimation ? new Runnable() {
							public void run() {
								mBackgroundContainer.hideBackground();
								mSwiping = false;
								mAnimating = false;
	...
	/**
	 * Animate a view between start and end X/Y locations, using either old
	 * (pre-3.0) or new animation APIs.
	 */
	@SuppressLint("NewApi")
	private void moveView(View view, float startX, float endX, float startY,
			float endY, Runnable endAction) {
		...
		if (isRuntimePostGingerbread()) {
			...
			if (startX != endX) {
				ObjectAnimator anim = ObjectAnimator.ofFloat(view,
						View.TRANSLATION_X, startX, endX);
			...
			if (startY != endY) {
				ObjectAnimator anim = ObjectAnimator.ofFloat(view,
						View.TRANSLATION_Y, startY, endY);
				anim.setDuration(MOVE_DURATION);
				anim.start();
				setAnimatorEndAction(anim, endAction);
			}
		} else {
			TranslateAnimation translator = new TranslateAnimation(startX,
					endX, startY, endY);
			translator.setDuration(MOVE_DURATION);
			view.startAnimation(translator);
			if (endAction != null) {
				view.getAnimation().setAnimationListener(
	...
	@SuppressLint("NewApi")
	private void setAnimatorEndAction(Animator animator,
	...
	private void setAnimationEndAction(Animation animation,
			final Runnable endAction) {
		if (endAction != null) {
	...
	/**
	 * Utility, to avoid having to implement every method in AnimationListener
